...
def get_hs300_components() -> List[str]:
    return ["600000.SH", "600519.SH", "000001.SZ", "000651.SZ"]

def load_instrument(symbol: str) -> Equity:
    venue = Venue("CSI300") if symbol == "HS300" else Venue("SSE")
    return Equity(
...
def aggregate_bars(bars_1min: List[Bar], target_minutes: int, instrument_id: InstrumentId) -> List[Bar]:
    if not bars_1min:
        return []
    result = []
    group = []
    bar_spec = BarSpecification(target_minutes, BarAggregation.MINUTE, PriceType.LAST)
    bar_type = BarType(instrument_id, bar_spec, AggregationSource.EXTERNAL)
    ...
    for symbol in symbols:
        ...
        for freq in frequencies:
            if freq == "1min":
                ...
                bars_dict[symbol][freq] = aggregate_bars(bars_1min, freq_map[freq], instrument_id)
...
def prepare_hs300_index_multi_period_data(frequencies: List[str]):
    ...
    for freq in frequencies:
        if freq == "1min":
            ...
            bars_dict[freq] = aggregate_bars(bars_1min, freq_map[freq], instrument_id)
    return bars_dict
